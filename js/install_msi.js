...
const install = async (info, excludes = [], preferPath, msiParams) => {
  const install = () => {
    ...
    const list = require('./walk')(`unzip\\${info.name}`);
    ...
    if (preferPath) {
      ...
      rootPath = list.filter(i => {
        ...
        for (let i = 1; i <= pathSplit.length; i++) {
          const a = pathSplit[pathSplit.length - i].toLowerCase();
          const b = _path[_path.length - i].toLowerCase();
          if (a !== b) return false;
        }
        return true;
      });
