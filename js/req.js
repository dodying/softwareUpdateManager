...
let initConfig = (obj) => { config = Object.assign(config, obj) }
let setConfig = (key, value) => { config[key] = value }
let getConfig = (key, defaultValue) => config[key] || defaultValue
...
function reqOption (uriOrOption, optionUser = {}) {
  ...
  if (useProxy && config._.request.proxy && config._.request.proxy.match(/(http|socks5):\/\/((.*?):(.*?)@)?(.*?):(\d+)/i)) {
    let [, protocol,, username, password, hostname, port] = config._.request.proxy.match(/(http|socks5):\/\/((.*?):(.*?)@)?(.*?):(\d+)/i)
...
async function reqHEAD (uriOrOption, optionUser = {}) {
  ...
  let head = async () => {
    return new Promise((resolve, reject) => {
      let req
      let _withoutHeader = config._.download.urlWithoutHeader.some(urlfilter => uri.match(urlfilter)) || withoutHeader
      if (!_withoutHeader) {
