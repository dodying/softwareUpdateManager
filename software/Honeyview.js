...
let data = {
  ...
  install: function (output, iPath) {
    ...
    if (shell) cp.execSync(`plugins\\RegDll64.exe /calldll "${parentPath}\\HVShell64.dll" unregSvr`)
    let portable = shell ? false : readlineSync.keyInYNStrict('Make it portable?')
    let installed = require('./../js/install')(output, iPath)
    if (installed) {
      if (shell) cp.execSync(`plugins\\RegDll64.exe /calldll "${parentPath}\\HVShell64.dll" RegSvr`)
      if (portable) {
