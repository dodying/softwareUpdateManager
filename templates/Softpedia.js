...
const data = {
  ...
  version: async (res, $, fns, choice) => {
    let version;
    const versionAll = res.body.match(/spjs_prog_version="(.*?)";/)[1];
    if (versionAll) {
      ...
      if (choice) {
        ...
        if (versionArr.length) {
          const matched = versionArr[0].match(choice);
  ...
  download: async (res, $, fns, choice) => {
    const tsf = res.body.match(/var spjs_prog_tsf=(.*);/)[1] * 1;
